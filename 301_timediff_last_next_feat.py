...
def pre_process(data):
    df = data[['shop_id', 'item_brand_id', 'item_id', 'item_category_1', 'item_pv_level', 'item_sales_level',
               'item_collected_level',
               'item_price_level', 'context_page_id', 'day', 'hour', 'maphour', 'context_timestamp', 'user_id',
               'instance_id']]
    del data
    gc.collect()
...
def user_check(df, behaviour):
    ...
    user_day = df.groupby(['user_id', 'day', behaviour]).size().reset_index().rename(
        columns={0: 'user_id_query_day_{}'.format(behaviour)})
    df = pd.merge(df, user_day, how='left', on=['user_id', 'day', behaviour])
    user_day_hour = df.groupby(['user_id', 'day', 'hour', behaviour]).size().reset_index().rename(
        columns={0: 'user_id_query_day_hour_{}'.format(behaviour)})
    df = pd.merge(df, user_day_hour, how='left', on=['user_id', 'day', 'hour', behaviour])
    user_day_hour_map = df.groupby(['user_id', 'day', 'maphour', behaviour]).size().reset_index().rename(
        columns={0: 'user_id_query_day_hour_map_{}'.format(behaviour)})
    ...
    # 今天截至目前查看的次数与当日查看总次数的比值
    df['check_{}_ratio'.format(behaviour)] = df['check_{}_time_day'.format(behaviour)] / df[
        'user_id_query_day_{}'.format(behaviour)]
...
def main():
    ...
    for f in ['shop_id', 'item_brand_id', 'item_id', 'item_category_1', 'item_pv_level', 'item_sales_level',
              'item_collected_level',
